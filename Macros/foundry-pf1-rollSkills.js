const config = {
  skills: ["knw", "acr", "apr", "blf", "clm", "crf", "dip", "dev", 
          ...
          "ste", "sur", "swm", "umd"],
  labels: ["Knowledge"],
  blindrollSkills: ["apr", "blf", "dis", "dev", "apr", "per"],
...
async function _rollSkill(skillId, actor) {
    ...
    if (config.blindrollSkills.indexOf(skillId) !== -1) {
        blindRoll = true;
    } 

    if (blindRoll) {
        // Set rollMode to blindroll
    ...
    // Reset rollMode to previously saved rollMode
...
function actorsSkill(actors) {
  ...
  const skillButtons = config.skills.reduce((buttons, skillId, index) => {
    ...
    if (actors.length === 1 && !config.withSubskills.includes(skillId) && label !== config.unknownSkill) {
      const skill = actors[0].getSkillInfo(skillId);
      const mod = skill.mod;
...
function actorSubSkill(actor, skillName, skill) {
  ...
  const subSkills = actor.getSkillInfo(skill).subSkills;
